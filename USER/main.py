...
def state_switch(red_blobs, green_blobs, black_line):
    ...
    if not red_blobs:
        ...
        if 0 <= (red_blobs[0]).cx() <= 40:
            state = states["SEARCHING_LEFT"]

        elif 80 < (red_blobs[0]).cx():
            ...
            if not green_blobs:
                ...
                # 红球在中间，且找到球门，但球可能很远
                #green_blob_rotation_deg = rotation_deg_filtered(green_blobs[0])  
                if black_line == blackline_states["CAR_TO_LEFT"]:
                    ...
                    #处于射门角度范围内，排除在过左或过右的情况
                    if abs((green_blobs[0]).cx() - IMG_WIDTH/2) < 10:
                        # 球门在正中间
                        if (green_blobs[0]).w() > 90: 
                        # 距离球门较近
                        #and  (green_blob_rotation_deg >= 150 or green_blob_rotation_deg <= 30)):
                            state = states["SHOOTING"]
                        else:
                            state = states["DRIBBLING_FORWARD"]
                        
                    elif abs((green_blobs[0]).cx() - IMG_WIDTH/2) < 30:
                        # 球门靠近中间
                        if (green_blobs[0]).w() > 90 :
                            # and (green_blob_rotation_deg >= 150 or green_blob_rotation_deg <= 30)):
                            state = states["SHOOTING"]  
...
while (True):
        ...
        if (state == states["SEARCHING"]):
            ...
            speed_L = 0
            speed_R = 0
            speed_B = sign((green_blobs[0]).cx()-(red_blobs[0]).cx()) * 1000
